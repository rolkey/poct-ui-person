        ...
        <el-row ref="editButtonsRef" :gutter="10" class="mb-[6px]">
          <el-col :span="1.5">
            <el-button
              ...
              icon="Plus"
              @click="handleAdd"
              v-hasPermi="['his:personAuthorizeItem:add']"
              >新增</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['his:personAuthorizeItem:edit']"
              >修改</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['his:personAuthorizeItem:remove']"
              >删除</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              icon="Download"
              @click="handleExport"
              v-hasPermi="['his:personAuthorizeItem:export']"
              >导出</el-button
            >
          </el-col>
...
import {
  listPersonAuthorizeItem,
  getPersonAuthorizeItem,
  delPersonAuthorizeItem,
  addPersonAuthorizeItem,
  updatePersonAuthorizeItem,
} from "@/api/lis/person/personAuthorizeItem";
import {
  PersonAuthorizeItemVO,
  PersonAuthorizeItemQuery,
  PersonAuthorizeItemForm,
} from "@/api/lis/person/personAuthorizeItem/types";
...
const data = reactive<PageData<PersonAuthorizeItemForm, PersonAuthorizeItemQuery>>({
  ...
  rules: {
    authId: [{ required: true, message: "授权ID不能为空", trigger: "blur" }],
  },
});
...
const handleDelete = async (row?: PersonAuthorizeItemVO) => {
  ...
  await proxy?.$modal
    .confirm('是否确认删除授权明细编号为"' + _itemIds + '"的数据项？')
...
const handleExport = () => {
  proxy?.download(
    "his/personAuthorizeItem/export",
    {
      ...queryParams.value,
    },
