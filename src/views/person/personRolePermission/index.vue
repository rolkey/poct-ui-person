        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item label="操作权限集合，多选逗号分隔" prop="operPerm">
              <el-input v-model="queryParams.operPerm" placeholder="请输入操作权限集合，多选逗号分隔" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="数据权限：本科室/全院" prop="dataScope">
              <el-input v-model="queryParams.dataScope" placeholder="请输入数据权限：本科室/全院" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never" class="flex flex-col flex-1 table-card">
      <template #header>
        <el-row ref="editButtonsRef" :gutter="10" class="mb-[6px]">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['his:personRolePermission:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['his:personRolePermission:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['his:personRolePermission:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['his:personRolePermission:export']">导出</el-button>
          </el-col>
      ...
      <div ref="tableWrapperRef" class="flex flex-row flex-1 overflow-hidden">
        <el-table v-loading="loading" border :data="personRolePermissionList" :height="tableHeight" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width">
            <template #default="scope">
              <el-tooltip content="修改" placement="top">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['his:personRolePermission:edit']"></el-button>
              </el-tooltip>
              <el-tooltip content="删除" placement="top">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['his:personRolePermission:remove']"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
...
import { listPersonRolePermission, getPersonRolePermission, delPersonRolePermission, addPersonRolePermission, updatePersonRolePermission } from '@/api/his/personRolePermission';
import { PersonRolePermissionVO, PersonRolePermissionQuery, PersonRolePermissionForm } from '@/api/his/personRolePermission/types';
...
const handleDelete = async (row?: PersonRolePermissionVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除报告权限管理编号为"' + _permIds + '"的数据项？').finally(() => loading.value = false);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('his/personRolePermission/export', {
    ...queryParams.value
