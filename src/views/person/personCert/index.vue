        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="发证机构" prop="issuer">
              <el-input v-model="queryParams.issuer" placeholder="请输入发证机构" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never" class="flex flex-col flex-1 table-card">
      <template #header>
        <el-row ref="editButtonsRef" :gutter="10" class="mb-[6px]">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['his:personCert:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['his:personCert:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['his:personCert:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['his:personCert:export']">导出</el-button>
          </el-col>
      ...
      <div ref="tableWrapperRef" class="flex flex-row flex-1 overflow-hidden">
        <el-table v-loading="loading" border :data="personCertList" :height="tableHeight" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="证书类型：执业医师/护士/技术资格/POCT专项/仪器认证" align="center" prop="certType" />
          ...
          <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width">
            <template #default="scope">
              <el-tooltip content="修改" placement="top">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['his:personCert:edit']"></el-button>
              </el-tooltip>
              <el-tooltip content="删除" placement="top">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['his:personCert:remove']"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
...
import { listPersonCert, getPersonCert, delPersonCert, addPersonCert, updatePersonCert } from '@/api/his/personCert';
import { PersonCertVO, PersonCertQuery, PersonCertForm } from '@/api/his/personCert/types';
...
const data = reactive<PageData<PersonCertForm, PersonCertQuery>>({
  ...
  rules: {
    ...
    certType: [
      { required: true, message: "证书类型：执业医师/护士/技术资格/POCT专项/仪器认证不能为空", trigger: "change" }
    ],
  }
});
...
const handleDelete = async (row?: PersonCertVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除资质证书编号为"' + _certIds + '"的数据项？').finally(() => loading.value = false);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('his/personCert/export', {
    ...queryParams.value
