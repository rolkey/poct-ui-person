        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item label="排序号" prop="sortNo">
              <el-input v-model="queryParams.sortNo" placeholder="请输入排序号" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="本题分值" prop="score">
              <el-input v-model="queryParams.score" placeholder="请输入本题分值" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item>
    ...
    <el-card shadow="never" class="flex flex-col flex-1 table-card">
      <template #header>
        <el-row ref="editButtonsRef" :gutter="10" class="mb-[6px]">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['his:personExamPaperQuestion:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['his:personExamPaperQuestion:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['his:personExamPaperQuestion:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['his:personExamPaperQuestion:export']">导出</el-button>
          </el-col>
      ...
      <div ref="tableWrapperRef" class="flex flex-row flex-1 overflow-hidden">
        <el-table v-loading="loading" border :data="personExamPaperQuestionList" :height="tableHeight" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width">
            <template #default="scope">
              <el-tooltip content="修改" placement="top">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['his:personExamPaperQuestion:edit']"></el-button>
              </el-tooltip>
              <el-tooltip content="删除" placement="top">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['his:personExamPaperQuestion:remove']"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
...
import { listPersonExamPaperQuestion, getPersonExamPaperQuestion, delPersonExamPaperQuestion, addPersonExamPaperQuestion, updatePersonExamPaperQuestion } from '@/api/his/personExamPaperQuestion';
import { PersonExamPaperQuestionVO, PersonExamPaperQuestionQuery, PersonExamPaperQuestionForm } from '@/api/his/personExamPaperQuestion/types';
...
const data = reactive<PageData<PersonExamPaperQuestionForm, PersonExamPaperQuestionQuery>>({
  ...
  rules: {
    ...
    score: [
      { required: true, message: "本题分值不能为空", trigger: "blur" }
    ],
  }
});
...
const submitForm = () => {
  personExamPaperQuestionFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        await updatePersonExamPaperQuestion(form.value).finally(() =>  buttonLoading.value = false);
      } else {
...
const handleDelete = async (row?: PersonExamPaperQuestionVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除试卷题目关联编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('his/personExamPaperQuestion/export', {
    ...queryParams.value
