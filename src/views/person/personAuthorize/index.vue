    ...
    <el-card shadow="never" class="flex flex-col flex-1 table-card">
      <template #header>
        <el-row ref="editButtonsRef" :gutter="10" class="mb-[6px]">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['his:personAuthorize:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['his:personAuthorize:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['his:personAuthorize:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['his:personAuthorize:export']">导出</el-button>
          </el-col>
      ...
      <div ref="tableWrapperRef" class="flex flex-row flex-1 overflow-hidden">
        <el-table v-loading="loading" border :data="personAuthorizeList" :height="tableHeight" @selection-change="handleSelectionChange">
          <el-table-column type="selection" width="55" align="center" />
          <el-table-column label="授权ID" align="center" prop="authId" v-if="false" />
          <el-table-column label="授权类型：操作授权/报告权限/质控权限/管理权限" align="center" prop="authType" />
          ...
          <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width">
            <template #default="scope">
              <el-tooltip content="修改" placement="top">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['his:personAuthorize:edit']"></el-button>
              </el-tooltip>
              <el-tooltip content="删除" placement="top">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['his:personAuthorize:remove']"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
...
import { listPersonAuthorize, getPersonAuthorize, delPersonAuthorize, addPersonAuthorize, updatePersonAuthorize } from '@/api/his/personAuthorize';
import { PersonAuthorizeVO, PersonAuthorizeQuery, PersonAuthorizeForm } from '@/api/his/personAuthorize/types';
...
const data = reactive<PageData<PersonAuthorizeForm, PersonAuthorizeQuery>>({
  ...
  rules: {
    authType: [
      { required: true, message: "授权类型：操作授权/报告权限/质控权限/管理权限不能为空", trigger: "change" }
    ],
  }
});
...
const handleDelete = async (row?: PersonAuthorizeVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除授权记录编号为"' + _authIds + '"的数据项？').finally(() => loading.value = false);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('his/personAuthorize/export', {
    ...queryParams.value
