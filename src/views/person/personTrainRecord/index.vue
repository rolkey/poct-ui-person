        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="培训计划ID" prop="planId">
              <el-input v-model="queryParams.planId" placeholder="请输入培训计划ID" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="培训时长" prop="hours">
              <el-input v-model="queryParams.hours" placeholder="请输入培训时长" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="学习进度" prop="progress">
              <el-input v-model="queryParams.progress" placeholder="请输入学习进度" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never" class="flex flex-col flex-1 table-card">
      <template #header>
        <el-row ref="editButtonsRef" :gutter="10" class="mb-[6px]">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['his:personTrainRecord:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['his:personTrainRecord:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['his:personTrainRecord:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['his:personTrainRecord:export']">导出</el-button>
          </el-col>
      ...
      <div ref="tableWrapperRef" class="flex flex-row flex-1 overflow-hidden">
        <el-table v-loading="loading" border :data="personTrainRecordList" :height="tableHeight" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width">
            <template #default="scope">
              <el-tooltip content="修改" placement="top">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['his:personTrainRecord:edit']"></el-button>
              </el-tooltip>
              <el-tooltip content="删除" placement="top">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['his:personTrainRecord:remove']"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
...
import { listPersonTrainRecord, getPersonTrainRecord, delPersonTrainRecord, addPersonTrainRecord, updatePersonTrainRecord } from '@/api/his/personTrainRecord';
import { PersonTrainRecordVO, PersonTrainRecordQuery, PersonTrainRecordForm } from '@/api/his/personTrainRecord/types';
...
const handleDelete = async (row?: PersonTrainRecordVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除培训记录编号为"' + _recordIds + '"的数据项？').finally(() => loading.value = false);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('his/personTrainRecord/export', {
    ...queryParams.value
