        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="正确答案" prop="answer">
              <el-input v-model="queryParams.answer" placeholder="请输入正确答案" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="难度系数1-5" prop="difficulty">
              <el-input v-model="queryParams.difficulty" placeholder="请输入难度系数1-5" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="题目分类" prop="category">
              <el-input v-model="queryParams.category" placeholder="请输入题目分类" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item>
    ...
    <el-card shadow="never" class="flex flex-col flex-1 table-card">
      <template #header>
        <el-row ref="editButtonsRef" :gutter="10" class="mb-[6px]">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['his:personExamQuestion:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['his:personExamQuestion:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['his:personExamQuestion:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['his:personExamQuestion:export']">导出</el-button>
          </el-col>
      ...
      <div ref="tableWrapperRef" class="flex flex-row flex-1 overflow-hidden">
        <el-table v-loading="loading" border :data="personExamQuestionList" :height="tableHeight" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width">
            <template #default="scope">
              <el-tooltip content="修改" placement="top">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['his:personExamQuestion:edit']"></el-button>
              </el-tooltip>
              <el-tooltip content="删除" placement="top">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['his:personExamQuestion:remove']"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
...
import { listPersonExamQuestion, getPersonExamQuestion, delPersonExamQuestion, addPersonExamQuestion, updatePersonExamQuestion } from '@/api/his/personExamQuestion';
import { PersonExamQuestionVO, PersonExamQuestionQuery, PersonExamQuestionForm } from '@/api/his/personExamQuestion/types';
...
const data = reactive<PageData<PersonExamQuestionForm, PersonExamQuestionQuery>>({
  ...
  rules: {
    questionType: [
      { required: true, message: "单选/多选/判断/操作不能为空", trigger: "change" }
    ],
    content: [
...
const handleDelete = async (row?: PersonExamQuestionVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除题库编号为"' + _questionIds + '"的数据项？').finally(() => loading.value = false);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('his/personExamQuestion/export', {
    ...queryParams.value
