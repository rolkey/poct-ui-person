        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="季度" prop="quarter">
              <el-input v-model="queryParams.quarter" placeholder="请输入季度" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never" class="flex flex-col flex-1 table-card">
      <template #header>
        <el-row ref="editButtonsRef" :gutter="10" class="mb-[6px]">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['his:personTrainPlan:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['his:personTrainPlan:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['his:personTrainPlan:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['his:personTrainPlan:export']">导出</el-button>
          </el-col>
      ...
      <div ref="tableWrapperRef" class="flex flex-row flex-1 overflow-hidden">
        <el-table v-loading="loading" border :data="personTrainPlanList" :height="tableHeight" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width">
            <template #default="scope">
              <el-tooltip content="修改" placement="top">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['his:personTrainPlan:edit']"></el-button>
              </el-tooltip>
              <el-tooltip content="删除" placement="top">
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['his:personTrainPlan:remove']"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
...
import { listPersonTrainPlan, getPersonTrainPlan, delPersonTrainPlan, addPersonTrainPlan, updatePersonTrainPlan } from '@/api/his/personTrainPlan';
import { PersonTrainPlanVO, PersonTrainPlanQuery, PersonTrainPlanForm } from '@/api/his/personTrainPlan/types';
...
const handleDelete = async (row?: PersonTrainPlanVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除培训计划编号为"' + _planIds + '"的数据项？').finally(() => loading.value = false);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('his/personTrainPlan/export', {
    ...queryParams.value
