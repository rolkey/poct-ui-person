        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="答题详情" prop="answerSheet">
              <el-input
                v-model="queryParams.answerSheet"
                placeholder="请输入答题详情"
                clearable
                @keyup.enter="handleQuery"
              />
        ...
        <el-row ref="editButtonsRef" :gutter="10" class="mb-[6px]">
          ...
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="single"
              @click="handleUpdate()"
              v-hasPermi="['his:personExam:edit']"
              >修改</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['his:personExam:remove']"
              >删除</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              icon="Download"
              @click="handleExport"
              v-hasPermi="['his:personExam:export']"
              >导出</el-button
            >
          </el-col>
...
import {
  ...
  addPersonExam,
  updatePersonExam,
} from "@/api/lis/person/personExam";
import { PersonExamVO, PersonExamQuery, PersonExamForm } from "@/api/lis/person/personExam/types";
...
const data = reactive<PageData<PersonExamForm, PersonExamQuery>>({
  ...
  rules: {
    examId: [{ required: true, message: "考试记录ID不能为空", trigger: "blur" }],
...
const handleDelete = async (row?: PersonExamVO) => {
  ...
  await proxy?.$modal
    .confirm('是否确认删除考试记录编号为"' + _examIds + '"的数据项？')
